...
def define_setup_dir(process_id, comment, algorithm, will_train, creation_path, use_all_dp0_dataset, env):
    global ENV
    ENV = env
...
def create_yaml_process_file(args):
    yaml_file = f'{args.creation_path}/{args.process_id}/process_info.yaml'
...
def create_yaml_pz_compute_train(args):
    yaml_file_config = f'{args.creation_path}/{args.process_id}/pz-train.yaml'
...
def create_yaml_pz_compute(args):
    yaml_file_config = f'{args.creation_path}/{args.process_id}/pz-compute.yaml'
...
def create_required_dirs(args):
    process_dir = f'{args.creation_path}/{args.process_id}' 
    try:
...
def add_input_data(args):
    if args.use_all_dp0_dataset:
        if os.path.exists(f'{args.creation_path}/{args.process_id}/input'):
            pattern = '*.hdf5'
            target_dir = f'{args.creation_path}/{args.process_id}/input/'
...
def copy_configs_file(args):
    ...
    dst = f"{args.creation_path}/{args.process_id}/"

...
cluster = SLURMCluster(
    ...
    job_extra_directives=[
        '--propagate',
        f'--output={logs_dir}/h1dmag_dask_job_%j.out',  # Redireciona a saída para a pasta output
        f'--error={logs_dir}/h1dmag_dask_job_%j.err'    # Redireciona o erro para a pasta output
    ],
)
...
performance_report_path = os.path.join(output_dir, f'performance_report_histo_1d_mag.html')
...
with performance_report(filename=performance_report_path):
    ...
    output_path = os.path.join(output_dir, 'histo_1d_mag_all_bands.parquet')

...
performance_report_path = os.path.join(output_dir, f'performance_report_{column_for_cut}_{value_for_cut}_{fraction}.html')

with performance_report(filename=performance_report_path):
    # Obter lista de arquivos HDF5 na pasta
    file_list = glob.glob('/lustre/t1/cl/lsst/dp01/primary/catalogs/truth/*.parquet')
    
    # Ler todos os arquivos parquet com dask.
    ddf = dd.read_parquet(file_list)
    
    # Remover valores NaN nas colunas de magnitude
    ddf = ddf.dropna(subset=[f'{column_for_cut}'])
    
    # Aplicar corte baseado na banda e magnitude fornecidas
    ddf_filtered = ddf[ddf[f'{column_for_cut}'] <= value_for_cut]
    
    # Fazer uma seleção aleatória.
    ddf_sampled = ddf_filtered.sample(frac=fraction)

    # Caminhos de saída
    output_path_parquet = os.path.join(output_dir, f'random_sample_{column_for_cut}_{value_for_cut}_{fraction}.parquet')
    output_path_csv = os.path.join(output_dir, f'random_sample_{column_for_cut}_{value_for_cut}_{fraction}.csv')
    output_path_hdf5 = os.path.join(output_dir, f'random_sample_{column_for_cut}_{value_for_cut}_{fraction}.hdf5')

...
logging.basicConfig(
    filename=os.path.join(logs_dir, 'histo_2d_color_color_debug.log'),
    level=logging.INFO,
...
cluster = SLURMCluster(
    ...
    job_extra_directives=[
        '--propagate',
        f'--output={logs_dir}/h2dcolorcolor_dask_job_%j.out',  # Redireciona a saída para a pasta output
        f'--error={logs_dir}/h2dcolorcolor_dask_job_%j.err'    # Redireciona o erro para a pasta output
    ],
)
...
performance_report_path = os.path.join(output_dir, f'performance_report_histo_2d_color_color.html')
...
bins = {
    'umg_x_gmr': (np.arange(-10, 10, 0.032), np.arange(-10, 10, 0.032)),
    'gmr_x_rmi': (np.arange(-10, 10, 0.032), np.arange(-10, 10, 0.032)),
    'rmi_x_imz': (np.arange(-10, 10, 0.032), np.arange(-10, 10, 0.032)),
    'imz_x_zmy': (np.arange(-10, 10, 0.032), np.arange(-10, 10, 0.032)),
}
###########################################################################################

################################ CÁLCULO DOS HISTOGRAMAS ##################################
def compute_total_histogram(dask_dataframe, color1_col, color2_col, graph):
    color1 = dask_dataframe[color1_col].to_dask_array()
    color2 = dask_dataframe[color2_col].to_dask_array()
    hist, _, _ = da.histogram2d(color1, color2, bins=bins[graph])
    return hist.compute()
...
with performance_report(filename=performance_report_path):
    ...
    for graph, (color1_col, color2_col) in graphs.items():
        total_histogram = compute_total_histogram(ddf, color1_col, color2_col, graph)
        all_data.append({'graph': graph, 'type': 'histogram', 'values': total_histogram.tolist()})
        all_data.append({'graph': graph, 'type': 'bins', 'values': {'color1_bins': bins[graph][0].tolist(), 'color2_bins': bins[graph][1].tolist()}})

    all_data_df = pd.DataFrame(all_data)

    output_path = os.path.join(output_dir, 'histo_2d_color_color_all_graphs.parquet')
